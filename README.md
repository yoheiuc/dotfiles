...
- `~/.claude/skills/` のうち `frontend-design` は dotfiles 管理（`home/dot_claude/skills/frontend-design/`、Apache-2.0 で Anthropic 公式 marketplace から vendor）。gws / playwright / notion / find-skills など外部 CLI で配布される skill は `post-setup.sh` が配置するので dotfiles 本体では管理しない
