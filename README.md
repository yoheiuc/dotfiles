...
- **同梱 skill**（`home/dot_claude/skills/`）: `screenshot` / `doc` / `pdf` / `spreadsheet` / `presentation` / `jupyter-notebook`
- **upstream install skill**（`scripts/post-setup.sh` が `npx skills add` で配置）: `gws-*` / `find-skills` / `playwright-cli` / `notion-cli` / `security-best-practices` / `ui-ux-pro-max`
...
│   │   └── skills/                 # 同梱 skill 6 個（screenshot / doc / pdf / spreadsheet / presentation / jupyter-notebook）
│   ├── dot_local/
