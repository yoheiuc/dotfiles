...
| 状況 | 採用方式 | 例 |
|---|---|---|
| 公式 CLI + 公式 skill が揃っている | **CLI + skill**（`scripts/post-setup.sh` で install） | `playwright-cli`、`gws` |
| 公式 CLI なし、公式 remote MCP がある（OAuth 認証） | **remote HTTP MCP**（`scripts/ai-repair.sh` で `~/.claude.json` に upsert） | Slack、Notion、Exa、Jamf docs |
...
2. **upstream の公式 CLI が提供する skill 配布**（gws / playwright 等）: `scripts/post-setup.sh` の `npx skills add ...` で `~/.claude/skills/` に install。dotfiles source には入れない
...
| `~/.claude/settings.json` の baseline 4 key（`autoUpdatesChannel` / `env.ENABLE_TOOL_SEARCH` / `hooks` / `effortLevel`） | dotfiles baseline | `scripts/ai-repair.sh` の upsert ロジックを編集。実体は Claude Code が rewrite する前提（`/effort` で local 上書き可、`make ai-repair` で `medium` に snap back） |
| `~/.claude/settings.json` のそれ以外（`permissions` / `model` / `statusLine`） | local 自由 | 触らない（Claude Code が rewrite） |
| `~/.claude/skills/{gws-*,find-skills,playwright-cli}` | post-setup install (skill) | `scripts/post-setup.sh` の install 句を編集。`npx skills add` 経由 |
...
`alwaysLoad: true`（Claude Code v2.1.121, 2026-04-28）は per-MCP の eager-load 指定で、tool schema を session 起動時に展開する（baseline は `ENABLE_TOOL_SEARCH=auto:5` で全体 deferred）。**毎セッション必ず使う頻用 MCP** にだけ付ける（現状: `slack` / `notion`）。`vision` / `sequential-thinking` のように起動コストの高い stdio 系や使用頻度の低い MCP には付けない。`_upsert_http_mcp` の第 4 引数 / `claude_mcp_http_matches` の第 4 引数で expected 値を指定し（"True" / 空）、drift 検知に組み込む。
...
- **公式 CLI で配布される** skill（gws / playwright 等）: `scripts/post-setup.sh` が `~/.claude/skills/` に install。dotfiles source には vendor しない
