...
- 全テスト: `make test`。単体テスト 1 本だけは `bash tests/<name>.sh` — 編集箇所別によく叩くのは `bash tests/ai-repair.sh`（MCP / hooks / 廃止 cleanup を変更時）/ `bash tests/post-setup.sh`（plugin / skill install を変更時）/ `bash tests/doctor.sh`（診断項目を変更時）/ `bash tests/playwright-zsh.sh`（zsh wrapper 編集時）
  - `tests/playwright-zsh.sh` は呼び出し元の env を完全 isolate しないため、親 shell に `PLAYWRIGHT_CLI_SESSION` が export されていると leak で fail する。回帰確認は `env -u PLAYWRIGHT_CLI_SESSION make test` で実行（恒久対策はテスト側を `env -i` に直すこと、未対応）
...
- 過去の判断を辿るとき: [`docs/notes/decisions-archive.md`](docs/notes/decisions-archive.md) を見る
...
判断記録は [`docs/notes/decisions-archive.md`](docs/notes/decisions-archive.md) に集約。新規判断は同 archive 末尾に追記する。`status` 凡例は archive 冒頭にある。
