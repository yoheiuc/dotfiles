...
- **Claude Code / Codex / Gemini 三者同居**: `settings.json` / `config.toml` に同じ model / sandbox / approval / hooks を揃え、MCP baseline（共通: Serena / Exa / Slack / Vision、Claude 専用: sequential-thinking、Codex 専用: OpenAI Developer Docs）を両方に配る。drift は `make ai-repair` で期待値に戻す
...
- **drift は自己修復**: `make ai-repair` が Claude `~/.claude.json` / Codex `~/.codex/config.toml` / Serena config / channel を baseline に戻し、legacy MCP（`playwright` / `filesystem` / `drawio` / `notion` / `github` / `owlocr` / `chrome-devtools` / `brave-search`）を能動削除
...
Claude / Codex 共通の MCP baseline を `home/dot_claude/dot_mcp.json`（HTTP MCP）と `home/dot_codex/config.toml.tmpl`（Codex 側の MCP 登録）に反映しています。stdio MCP は `scripts/post-setup.sh` が `~/.claude.json` に直接 upsert するので、`dot_mcp.json` には載りません。

**両方に登録**（HTTP MCP、`dot_mcp.json` + `config.toml.tmpl`）
...
**Claude 専用**（stdio MCP、`post-setup.sh` が `~/.claude.json` に登録）

- `sequential-thinking`（`@modelcontextprotocol/server-sequential-thinking`）

**Codex 専用**（`config.toml.tmpl` の `[mcp_servers.openaiDeveloperDocs]`）

- `OpenAI Developer Docs` — Codex から OpenAI API のリファレンスを引くために登録。Claude 側には入れていない（Claude は Anthropic 文書を直接参照できるため）。
...
Claude Code は `settings.json` を自身で頻繁に書き換えるため、dotfiles が全体を所有すると必ず drift する。そこで **baseline キーだけ** を `make ai-repair` が upsert し、残りはローカル管理にしている。Codex / Gemini も同じ原則で分けている。
...
| `~/AGENTS.md` / `~/.codex/config.toml` / `hooks.json` / `skills/` | dotfiles |
| `~/.codex/auth.json` / `sessions/` / `history.jsonl` / `rules/` / `memories/` etc. | 管理しない |
| `~/.gemini/settings.json` | dotfiles |
| `~/.gemini/oauth_creds.json` / `history/` / `state.json` etc. | 管理しない |
...
`~/.codex/config.toml` は chezmoi テンプレート管理。`make ai-repair` が保証する baseline:

- `model = "gpt-5.4"`、`model_reasoning_effort = "medium"`、`personality = "pragmatic"`
- `approval_policy = "on-request"` + `sandbox_mode = "workspace-write"`（`--full-auto` 相当）
- `[features]`: `multi_agent = true`、`codex_hooks = true`
- `[plugins]`: Google Calendar / GitHub / Gmail / Google Drive / build-macos-apps / build-ios-apps（Notion は `ntn` CLI + `makenotion/skills` に移行済み）
- MCP: Serena / vision / exa / slack / OpenAI Developer Docs
- マシン固有パスは `{{ .chezmoi.homeDir }}` で展開

`~/.codex/hooks.json` の Stop フックは `codex-auto-save-memory` skill を実行する。

### Serena MCP

Claude / Codex 両方で使う前提。`~/.local/bin/serena-mcp` wrapper 経由で起動し、Homebrew の `uvx` とブラウザ自動起動抑止を明示している。`~/.serena/serena_config.yml` 本体はローカル state だが、`make status` / `make ai-audit` / `make doctor` で主要キー（`language_backend: LSP` / dashboard / `project_serena_folder_location`）を監査する。
...
- Serena wrapper / OpenAI Docs MCP / exa / slack / vision の登録有無（legacy MCP: `playwright` / `filesystem` / `drawio` / `notion` / `github` / `owlocr` / `chrome-devtools` / `brave-search` が残っていれば warning）
- Serena config の主要キー
...
- **Codex CLI** は公式 npm パッケージ経由で `post-setup.sh` が導入
- **clasp** は `post-setup.sh` が `npm install -g @google/clasp`。初回 `clasp login` が必要
- **Sequential Thinking MCP** / **gws skills** / **find-skills**（`vercel-labs/skills`）は `post-setup.sh` が Claude Code / Codex 両方に配置
- **同梱 skill**（`home/dot_codex/skills/`）: `screenshot` / `doc` / `pdf` / `spreadsheet` / `jupyter-notebook` / `security-best-practices` / `ui-ux-pro-max` / `codex-auto-save-memory`
...
Gemini は one-shot 用途の zsh 関数を用意:

- `gr "<prompt>"` = `gemini -p`
- `gmr` / `gms` / `gmd` = 現在の git diff を review / summarize / explain
...
│   │   └── skills/frontend-design/ # Apache-2.0 vendor (anthropics/claude-plugins-official)
│   ├── dot_codex/
│   │   ├── config.toml.tmpl        # chezmoi template
│   │   ├── hooks.json              # Stop フックで auto-save-memory
│   │   └── skills/                 # 同梱 skill 8 個（post-setup が配置する gws / playwright は除く）
│   ├── dot_gemini/settings.json
│   ├── dot_local/
│   │   ├── bin/                    # ai-secrets / mcp-with-keychain-secret / serena-mcp
