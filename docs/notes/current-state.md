...
- 同梱 skill (vendored) は `screenshot`, `doc`, `pdf`, `spreadsheet`, `presentation`, `jupyter-notebook` の 6 種。`playwright` / `gws-*` / `find-skills` / `notion-cli` / `security-best-practices` / `ui-ux-pro-max` は `post-setup.sh` が upstream から `npx skills add` で配置するため dotfiles 本体では管理しない（tier 2 = upstream CLI 配布、vendor は最終手段）。2026-04-26 に `security-best-practices`（~8K 行）と `ui-ux-pro-max`（~1.5K 行）を vendor から upstream-install へ移行し、計 ~9.5K 行を dotfiles から退避した。
- Claude Code は `~/.claude/CLAUDE.md` を完全管理、`~/.claude/settings.json` は baseline 4 key（`autoUpdatesChannel` / `env.ENABLE_TOOL_SEARCH` / `effortLevel` / `hooks`）だけ `make ai-repair` で upsert し、`permissions` / `model` / `statusLine` はローカル管理。履歴や cache も管理しない。
