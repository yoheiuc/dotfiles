...
| `~/.claude/skills/{doc,jupyter-notebook,pdf,presentation,screenshot,spreadsheet,ui-ux-pro-max}` | chezmoi end-to-end (vendored) | 同上。marketplace に対応 plugin が現れたら plugin 化を検討して vendor を退避 |
| `~/.claude/skills/security-best-practices` | post-setup install (skill, upstream `tech-leads-club/agent-skills`) | `scripts/post-setup.sh` の install ブロックを編集。`npx skills add` 経由。marker file `.upstream-installed` で legacy vendored copy と区別 |
...
- 追加するなら以下を全部更新：
  ...
  - `README.md` の slash commands 一覧（無ければ section ごと再追加）
