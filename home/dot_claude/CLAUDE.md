...
- 可能ならその場で修正まで進める
- 破壊的な操作は確認なしで進めない
- **初回プロンプトは Goal / Constraints / Acceptance criteria の 3 点を一括で渡す**。途中介入で軌道修正するより初回投入の精度が上がる（Opus 4.7 系の長期自律実行設計に合わせた前提）
- 長時間自律実行は `/focus` で途中ログを隠して結果だけ見る運用にする（復帰時の Recaps は built-in、手動で要約させない）
