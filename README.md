...
- **MCP / CLI / 削除の判定マトリクス**: 新ツール追加前に [`CLAUDE.md`](./CLAUDE.md#ツール採用基準) を通す。MCP を選ぶのは symbol 解析や CoT scaffolding など agent context と不可分なものだけ
...
- [設計思想](#設計思想)（[`CLAUDE.md`](./CLAUDE.md) に採用基準・整合性・依存マップを集約）/ [セキュリティモデル](#セキュリティモデル)
...
| `make ai-audit` | ローカル管理の AI 設定だけを詳しく確認（CI 用途は `bash scripts/ai-audit.sh --quiet`：attention 行のみ stdout、attention があれば exit 1） |
...
採用基準マトリクス・整合性ルール・依存マップ・個別判断ログは [`CLAUDE.md`](./CLAUDE.md) に集約してある（このリポジトリで作業する Claude が会話開始時に読み込む）。新ツール追加・既存置換・廃止のいずれも、まず `CLAUDE.md` を見て該当する型を判定してから手を動かす。
...
**Claude Code plugin**（`post-setup.sh` が `claude plugin install ...@claude-plugins-official` で配置、per-user scope）

- LSP 群 — `clangd-lsp` / `csharp-lsp` / `gopls-lsp` / `jdtls-lsp` / `kotlin-lsp` / `lua-lsp` / `php-lsp` / `pyright-lsp` / `ruby-lsp` / `rust-analyzer-lsp` / `swift-lsp` / `typescript-lsp`（Anthropic 公式 marketplace の per-language LSP plugin 群。Claude Code 2.0.74 で入った native LSP tool 上に乗り、go-to-definition / find-references / rename / diagnostics を担う。2026-04-24 に `serena` MCP を retire してここへ移行済み）
- General 群 — `claude-md-management`（CLAUDE.md の audit / session learning capture）/ `claude-code-setup`（codebase から hooks / skills / MCP / subagent を提案）/ `feature-dev`（探索 → 設計 → レビューの subagent ワークフロー）/ `explanatory-output-style`（実装意図の説明を加えた output style、opt-in）

期待リストは `scripts/lib/claude-plugins.sh` の `CLAUDE_LSP_PLUGINS` / `CLAUDE_GENERAL_PLUGINS` で一元管理し、`make doctor` が `~/.claude/plugins/installed_plugins.json` と突き合わせて drift を検出する。
...
検索系は `Exa MCP`（`https://mcp.exa.ai/mcp?tools=web_search_exa,web_fetch_exa,web_search_advanced_exa`、API key 不要）に一本化しています。`?tools=` は **filter parameter** で、3 tool すべて明示すると `web_search_exa` / `web_fetch_exa` （default）に加え `web_search_advanced_exa`（domain / date / category filter）も有効になります。以前入れていた `brave-search`（`@modelcontextprotocol/server-brave-search`、Keychain 経由 API key）は Exa で十分カバーできたため 2026-04 に retire しました。`make ai-repair` が legacy 削除対象に入れているので、旧マシンでも自動で剥がれます。

Jamf 公式 docs MCP (`https://developer.jamf.com/mcp`、無認証 read-only) も baseline に含まれます。Jamf Pro の API 仕様検索を Claude Code から自然言語で引けます。実 Jamf Pro tenant 操作 / Netskope / Microsoft MCP for Enterprise 等の **業務 SaaS MCP は dotfiles 管理外**（tenant URL や API token が固有のため）。setup 手順だけ `CLAUDE.md` の「仕事用 MCP の setup 手順」節に集約してあるので、必要時に参照して手で install してください。
...
|---|---|---|
| Word | `doc` skill — `python-docx` + LibreOffice rendering | `gws-docs` skill — `gws docs` CLI |
| Excel | `spreadsheet` skill — `openpyxl` + LibreOffice rendering | `gws-sheets` skill — `gws sheets` CLI |
| PowerPoint | `presentation` skill — `python-pptx` + LibreOffice rendering | `gws-slides` skill — `gws slides` CLI |
| PDF | `pdf` skill — `reportlab` / `pdfplumber` / `pypdf` | （Drive 経由で扱う場合は `gws-drive`） |
...
- **同梱 skill**（`home/dot_claude/skills/`）: `screenshot` / `doc` / `pdf` / `spreadsheet` / `presentation` / `jupyter-notebook` / `ui-ux-pro-max`
- **upstream install skill**（`scripts/post-setup.sh` が `npx skills add` で配置）: `gws-*` / `find-skills` / `playwright-cli` / `notion-cli` / `security-best-practices`
- **claude-plugins-official 経由の plugin**（`post-setup.sh` が `claude plugin install ...@claude-plugins-official` で配置）:
  ...
  - 期待リストは `scripts/lib/claude-plugins.sh` の `CLAUDE_LSP_PLUGINS` / `CLAUDE_GENERAL_PLUGINS` で一元管理。`make doctor` が `~/.claude/plugins/installed_plugins.json` を見て drift を検出する
...
│   │   └── skills/                 # 同梱 skill 7 個（screenshot / doc / pdf / spreadsheet / presentation / jupyter-notebook / ui-ux-pro-max）
│   ├── dot_local/
...
4. **MCP セット**: 使わない MCP は `dot_mcp.json` から消し、`ai-repair.sh` の baseline と `ai-audit.sh` の legacy 削除対象を対応させる。新規追加は [`CLAUDE.md`](./CLAUDE.md#ツール採用基準) のマトリクスを先に通す。
...
- **設計判断に対する反論**: [`CLAUDE.md`](./CLAUDE.md#ツール採用基準) のマトリクスの枠組みで書かれた反論は読む。「好み」ベースは読まない。
