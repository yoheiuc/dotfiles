...
| 2026-04-26 | `home/dot_claude/commands/` を 18 → 3 本に整理（`perf` / `playwright` / `research` のみ残す） | L1 新ルール「Claude Code 標準機能で代替できないか先に確認」を適用。**built-in shadow** が 2 件（`/security-review` `/debug` は Claude Code が同名を持つ）、**自リポ skill との重複**が 7 件（`doc` / `notebook` / `pdf` / `presentation` / `screenshot` / `spreadsheet` / `ui-ux` は `~/.claude/skills/<対応名>/SKILL.md` の内容コピー、文言 drift も既に発生）、**汎用 methodology**が 6 件（`api-design` / `ci` / `diagram` / `docker` / `refactor` / `test` は Claude が一般知識として持つ）。残した 3 件は `pwattach` / `PLAYWRIGHT_CLI_SESSION=chrome` / Exa MCP tool 名等 dotfiles 固有 helper を語る部分があり代替不可。orphan target は `ai-repair.sh` の retired_command ループで能動削除（Codex / frontend-design retire と同パターン） | 本セッション、L1 `43b4fa7` ルール適用 |
...
`~/.claude/settings.json` の baseline key を増減・変更するときは以下を全部更新する。一つでも漏らすと L2 / README / scripts / tests のどこかが嘘になる：
...
- `README.md` の「Claude Code / MCP > 管理方針」表
- `README.md` の「Claude Code の baseline」 enumeration
...
新規追加するときの判断は L1「Claude Code 標準機能で代替できないか先に確認」を最優先で通す（built-in shadow / 自リポ skill 重複 / 汎用 methodology は custom 化しない）。**追加・削除どちらの場合も以下を全部更新**：

- `home/dot_claude/commands/<name>.md`: 追加 / 削除（chezmoi end-to-end 管理）
- `README.md` の「Slash commands」表（intro 文言の本数 + 表）
- `README.md` の Office 系比較表（`/doc` / `/spreadsheet` 等の slash 表記が table に入っているとき）
- `tests/commands.sh`: 閾値 `>= N` を実数に合わせる（floor は小さく保つ — 増加時に再検査が走る）
- 削除のときは追加で：
  - `scripts/ai-repair.sh`: retired_command ループの配列に追加（chezmoi は orphan target を自動削除しないので能動 rm が要る）
  - `tests/ai-repair.sh`: 該当 retired-state scenario に fixture（事前 touch）と assertion（事後 absent + announce）追加
- `CLAUDE.md`（L2）判断ログにエントリ（採用 / 削除どちらも理由を明文化）
