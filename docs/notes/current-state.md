...
- 同梱 skill (vendored) は `screenshot`, `doc`, `pdf`, `spreadsheet`, `presentation`, `jupyter-notebook`, `ui-ux-pro-max` の 7 種。`playwright` / `gws-*` / `find-skills` / `notion-cli` / `security-best-practices` は `post-setup.sh` が upstream から `npx skills add` で配置するため dotfiles 本体では管理しない（tier 2 = upstream CLI 配布、vendor は最終手段）。`security-best-practices` は 2026-04-26 に vendor から upstream-install へ移行（`tech-leads-club/agent-skills`）し、~8K 行を dotfiles から退避した。
