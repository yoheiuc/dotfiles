...
マトリクスでは決まらないケース（同じ機能を提供する複数の経路がある等）の判断記録。同じ議論を繰り返さないため。

- **Exa は HTTP MCP のまま、plugin 化しない**（2026-04-25）。`claude-plugins-official` に `exa` plugin が存在するが、stdio 版で API key と subprocess が要る。policy「remote MCP > local stdio MCP」と衝突。`web_search_exa` / `web_fetch_exa` の2 tool で日常用途は足りているので追加価値なし。
- **`chrome-devtools-mcp` plugin を採用しない**（2026-04）。実 Chrome に attach せず throwaway Chromium を spawn する設計で、`pwattach` 運用と相反する。
- **`code-review` / `pr-review-toolkit` plugin を採用しない**（2026-04-25）。既存の `/review` `/ultrareview` `/security-review` で同等の用途をカバー済み。
- **`code-simplifier` plugin を採用しない**（2026-04-25）。同等の `simplify` skill を vendoring 済み。
- **`commit-commands` plugin を採用しない**（2026-04-25）。HEREDOC + Co-Authored-By 等の独自コミット規約と衝突しやすい。
- **`superpowers` / `context7` plugin は dotfiles 管理外**（README.md に明文）。必要時にセッション内 `/plugin install` で個別投入する。dotfiles 必須セットには含めない。
