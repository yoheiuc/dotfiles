...
- 可能ならその場で修正まで進める
- 破壊的な操作は確認なしで進めない
- **初回プロンプトを受けたら Goal / Constraints / Acceptance criteria の 3 点を頭で組み立てて作業計画を立てる**。曖昧な点は最初に 1 度だけ確認、それ以降は途中介入を待たず一括処理する（Opus 4.7 系は途中介入での軌道修正より初回投入の精度が高い前提で動く）
- 長時間自律タスクの実行中は user が `/focus` で途中ログを隠している前提で、最終結果だけで判断できる粒度の出力を心がける（手動で Recaps 相当の要約を作らない、built-in に任せる）
...
`playwright-cli` を使う前に `echo "${PLAYWRIGHT_CLI_SESSION:-<none>}"` で session 状態を確認。値が `chrome` = `pwattach` で **user の実 Chrome に CDP attach 中** = 全タブが CDP 経由で操作可能 → 認証情報漏洩リスクが高いので以下を必ず守る:
