...
ブラウザ自動化の運用ルール（user が `pwedge` 起動 → Claude が `--session=edge` で attach する二段運用、禁止 click / eval パターン、AI 専用プロファイル分離）は L1 (`home/dot_claude/CLAUDE.md`) の「ブラウザ自動化のセキュリティ規則」「ブラウザ自動化の運用デフォルト」節が正本。L2 で wrapper / cheat / Brewfile の整合を扱うときは L1 と乖離していないか確認する。stealth 設定は `home/dot_playwright/cli.config.json`（chezmoi → `~/.playwright/cli.config.json`）が `launchOptions.args` / `ignoreDefaultArgs` を担い、整合チェックは `playwright_is_stealth_patched` predicate + `make doctor` で毎回 verify（採用経緯は archive 2026-04-28）。
