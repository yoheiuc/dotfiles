...
- 全テスト: `make test`。単体テスト 1 本だけは `bash tests/<name>.sh` — 編集箇所別によく叩くのは `bash tests/ai-repair.sh`（MCP / hooks / 廃止 cleanup を変更時）/ `bash tests/post-setup.sh`（plugin / skill install を変更時）/ `bash tests/doctor.sh`（診断項目を変更時）
- コミット規約: `<topic>: <動詞句>` + `Co-Authored-By: Claude Opus 4.7 ...` を HEREDOC で渡す（詳細は本ファイル「Commit message 規約」節）

詳細・セットアップ手順は `README.md`。

## このファイルの読み方

L2 は ~700 行ある。目的別に読む順序を決めると認知コストが下がる:

- 新規参入（このリポで何かする前に）: 「30 秒」→「ツール採用基準」→「スクリプトの責務境界」だけで作業開始可能
- 変更を加えるとき: 「整合性ルール」→「変更箇所の依存マップ」を必ず通す（更新漏れで scripts / tests / docs が矛盾する事故を防ぐ）
- 過去の判断を辿るとき: 「個別判断ログ」表 + 古いものは [`docs/notes/decisions-archive.md`](docs/notes/decisions-archive.md)
- 設定ファイルが何経由で管理されてるか迷ったとき: 「`~/.claude/` 配下の管理モード」表
...
| `claude-checks.sh` | Claude Code 設定の純粋 predicate（hook 登録 / MCP 登録 / autoupdate channel / plugin install 状態等の有無を bool で返す） | `ai-audit` / `doctor` |
| `claude-plugins.sh` | 期待 plugin リスト（`CLAUDE_LSP_PLUGINS` / `CLAUDE_GENERAL_PLUGINS` / `CLAUDE_DOCUMENT_PLUGINS`）と marketplace 名 + 単一 plugin の installed 判定（marketplace 引数対応）| `post-setup` (install) / `doctor` / `ai-audit` (verify) |
