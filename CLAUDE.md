...
- credential が必要な MCP は `~/.local/bin/mcp-with-keychain-secret` 経由で Keychain から注入（`security add-generic-password -s dotfiles.ai.mcp -a <account>` 直接 / 詳細は `docs/work-mcp-setup.md`）
- `tests/` 配下の対応テスト
