# dotfiles — Codex Notes

個人のツール採用ポリシー（MCP / CLI / 削除の判定、タスク別ツール選択、skills 一覧）は `~/AGENTS.md`（= `home/AGENTS.md`）に寄せた。ここにはこのリポジトリ固有の整合性ルールだけ置く。

## dotfiles 固有の整合性ルール

- dotfiles は再現性を優先し、認証情報や履歴などのローカル state は混ぜない
...
- `~/.codex/skills/`（dotfiles 管理）に置く skill は `home/dot_codex/skills/` に vendor
- `npx skills add` で配置する skill（gws / playwright / notion 等）は `~/.agents/skills/` に install、dotfiles には入れない
