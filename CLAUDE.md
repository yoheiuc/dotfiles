...
`adopted` / `rejected` で議論再開の見込みがないものは将来 `docs/notes/decisions-archive.md` に切り出して L2 を圧縮する（毎ターン context cost 削減）。
