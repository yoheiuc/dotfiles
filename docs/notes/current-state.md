...
- MCP baseline は `exa` / `brave-search` / `notion` / `slack` / `serena` / `chrome-devtools` / `sequential-thinking`。`notion` / `slack` は公式 remote（HTTP + OAuth）で、ローカル secret を持たない。ブラウザ操作は `@playwright/cli` + skill、ファイル操作は Claude Code の native tools、図は Mermaid（`.md` 直埋め or `mermaid-cli`）に寄せた。
