...
| 状況 | 採用方式 | 例 |
|---|---|---|
| 公式 CLI + 公式 skill が揃っている | **CLI + skill**（`scripts/post-setup.sh` で install） | `playwright-cli`、`ntn` |
...
1. **Anthropic 公式 / vetted marketplace の plugin**: `scripts/lib/claude-plugins.sh` の **配列 + marketplace 名のペア**に追加 → `make install` で自動配置。SHA pin で再現性あり、upstream rolling update も marketplace 経由で取り込める。現時点で使用中の marketplace は 1 個：
...
2. **upstream の公式 CLI が提供する skill 配布**（playwright / notion 等）: `scripts/post-setup.sh` の `npx skills add ...` で `~/.claude/skills/` に install。dotfiles source には入れない
...
#### global skill bulk install を入れない判断（2026-05-06）

`googleworkspace/cli`（gws-* / recipe-* / persona- 計 92 skill）と `document-skills@anthropic-agent-skills`（pdf/docx/pptx/xlsx 等 16 skill bundle）は **global には install しない**。理由は context cost: 全 Claude Code セッションで `name` + `description` が system prompt に常駐し、概算 ~10k token / turn を食う。利用頻度の高い特定プロジェクトでだけ project-scoped install で取る方が token 効率が良い。

必要なプロジェクトでの install:

```bash
# gws の特定 skill だけ project-scoped に：
cd <project>
npx skills add https://github.com/googleworkspace/cli -a claude-code -y --skill gws-gmail
npx skills add https://github.com/googleworkspace/cli -a claude-code -y --skill gws-calendar

# document-skills を project-scoped に（marketplace の登録自体も project スコープ）：
cd <project>
claude plugin marketplace add anthropics/skills
claude plugin install document-skills@anthropic-agent-skills
```

後者は `<project>/.claude/plugins/...` に展開され、その project のセッションでのみ load される。`ai-repair` は `~/.claude/skills/{gws,recipe,persona}-*` と `document-skills@anthropic-agent-skills` plugin / `anthropic-agent-skills` marketplace を能動削除する。
